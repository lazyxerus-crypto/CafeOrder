    ...
    private static void CheckDatabase()
    {
        ...
        using (var db = data.Store.Database.Connect())
        {
            ...
            Require(SqlNumber(db, "SELECT COUNT(*) FROM Products") == 0 && SqlNumber(db, "SELECT COUNT(*) FROM CartItems") == 0 &&
                SqlNumber(db, "SELECT COUNT(*) FROM Suppliers") == 7 && SqlNumber(db, "SELECT MAX(Version) FROM SchemaMigrations") == 5,
        ...
        using (var db = recovered.Store.Database.Connect()) Require(SqlNumber(db, "SELECT MAX(Version) FROM SchemaMigrations") == 5, "Failed import retries safely");
        ...
        Require(upgraded.Products.Single(p => p.Id == 25).LastSuccessfulCheckAtUtc == null &&
            ...
            Directory.GetFiles(Path.Combine(priorDirectory, "Data", "backups"), "*before-schema-5*.db").Length == 1,
        ...
        { db.Open(); using var cmd = db.CreateCommand(); cmd.CommandText = "CREATE TABLE SchemaMigrations(Version INTEGER PRIMARY KEY, Name TEXT NOT NULL, AppliedAt TEXT NOT NULL); INSERT INTO SchemaMigrations VALUES(6,'future','2026-01-01')"; cmd.ExecuteNonQuery(); }
        try { _ = new SampleData(new LocalState(futureDirectory)); throw new Exception("Future DB version silently changed"); }
        catch (InvalidDataException) { }
        using (var db = new SqliteConnection($"Data Source={futurePath}"))
        { db.Open(); Require(SqlNumber(db, "SELECT MAX(Version) FROM SchemaMigrations") == 6, "Future DB version remains unchanged"); }
