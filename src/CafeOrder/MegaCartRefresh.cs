...
public sealed partial class SampleData
{
        ...
        Func<string, Task<MegaProductLookupResult>> lookup)
    {
        ...
        try
        {
            ...
            do
            {
                ...
                Notify();
                await RefreshMegaCartOnceAsync(product, line, lookup);
    ...
    private async Task RefreshMegaCartOnceAsync(Product product, CartLine line,
        Func<string, Task<MegaProductLookupResult>> lookup)
    {
        string? goodsNo = ProductUrlIdentity.MegaGoodsNo(product.Supplier.Id, product.Url);
        Store.Log.Write(LogLevel.INFO, "CART_FIRST_LOOKUP_STARTED", "메가커피 상품 최초 담기 정보를 확인하기 시작했습니다.",
            supplier: "mega", productId: product.Id, goodsNo: goodsNo, result: "STARTED");
        ...
        catch (Exception ex) when (ex is not OutOfMemoryException)
        {
            ...
            Store.Log.Write(LogLevel.ERROR, "CART_FIRST_LOOKUP_FAILED", "메가커피 최초 담기 브라우저 조회에 실패했습니다.",
                supplier: "mega", productId: product.Id, goodsNo: goodsNo, result: "FAILED",
                reason: "LOOKUP_EXCEPTION", error: ex);
            return;
        }
        if (!Cart.Contains(line) || !Products.Contains(product) || !product.IsActive ||
            ProductUrlIdentity.MegaGoodsNo(product.Supplier.Id, product.Url) != goodsNo)
        {
            Store.Log.Write(LogLevel.INFO, "CART_FIRST_LOOKUP_DISCARDED", "조회 중 상품 또는 장바구니 상태가 바뀌어 결과를 반영하지 않았습니다.",
                supplier: "mega", productId: product.Id, goodsNo: goodsNo, result: "SKIPPED", reason: "STALE_CART_ADD");
            return;
        }
        if (result.Status != MegaProductLookupStatus.Success || result.Product is not { } snapshot)
        {
            ...
            Store.Log.Write(LogLevel.WARN, "CART_FIRST_LOOKUP_FAILED", "메가커피 최초 담기 상품정보를 확인하지 못해 저장값을 유지했습니다.",
                supplier: "mega", productId: product.Id, goodsNo: goodsNo, result: "FAILED", reason: reason);
            return;
        }
        if (string.IsNullOrWhiteSpace(snapshot.Name) || string.IsNullOrWhiteSpace(snapshot.DisplayPrice) ||
            ...
            ProductUrlIdentity.MegaGoodsNo("mega", snapshot.ProductUrl) != goodsNo)
        {
            megaCartFailed.Add(product.Id); Notify();
            Store.Log.Write(LogLevel.WARN, "CART_FIRST_LOOKUP_FAILED", "조회 결과에 필수 상품정보가 없어 저장값을 유지했습니다.",
                supplier: "mega", productId: product.Id, goodsNo: goodsNo, result: "FAILED", reason: "INCOMPLETE_PRODUCT");
            return;
        }
        ...
        try
        {
            ...
            if (oldPrice != product.Price)
                Store.Log.Write(LogLevel.INFO, "CART_FIRST_PRICE_CHANGED", "메가커피 조회 가격이 저장 가격과 달라 상품·장바구니 합계를 갱신했습니다.",
                    supplier: "mega", productId: product.Id, goodsNo: goodsNo, result: "CHANGED",
                    oldPrice: oldPrice, newPrice: product.Price);
            if (oldAvailable != product.Available)
                Store.Log.Write(LogLevel.INFO, "CART_FIRST_STOCK_CHANGED", "메가커피 조회 품절 상태가 변경됐습니다.",
                    supplier: "mega", productId: product.Id, goodsNo: goodsNo, result: product.Available ? "AVAILABLE" : "SOLD_OUT");
        ...
        catch (Exception ex) when (ex is not OutOfMemoryException)
        {
            if (!saved)
            {
                ...
                supplier: "mega", productId: product.Id, goodsNo: goodsNo, result: "FAILED", error: ex);
        }
        finally
        {
