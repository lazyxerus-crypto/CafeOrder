...
internal sealed partial class SupplierSessionManager
{
        ...
        CatalogDatabase database)
    {
        ...
        lock (sync)
        {
            ...
            var task = PrepareNuldamAfterAsync(prior, slot, attempt, database);
            slot.Running = task;
            return task;
        }
    }

    private async Task<SiteCartPreparationResult> PrepareNuldamAfterAsync(Task? prior, SessionSlot slot,
        ...
        try
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    { await vault.SaveAsync(context, login.HomeUrl); Publish("nuldam", SupplierLoginState.LoggedIn); }
                }
            }
            finally
            {
