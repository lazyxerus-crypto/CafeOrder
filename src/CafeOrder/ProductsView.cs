...
public sealed class ProductsView : UserControl
{
    ...
    public ProductsView(SampleData data)
    {
        ...
        var filters = new TableLayoutPanel { Name = "Filters", Dock = DockStyle.Top, AutoSize = true, ColumnCount = 8, RowCount = 1 };
        foreach (float percent in new[] { 37f, 21f, 24f, 18f })
        ...
        Control[] filterControls = [Ui.Text("검색"), search, Ui.Text("카테고리"), category, Ui.Text("발주처"), supplier, Ui.Text("정렬"), sort];
        ...
        var left = new BufferedPanel { Dock = DockStyle.Fill };
        left.Controls.Add(products); left.Controls.Add(count);
        var right = new BufferedPanel { Dock = DockStyle.Fill, Padding = new Padding(4, 0, 0, 0) };
        ...
        right.Controls.Add(cart); right.Controls.Add(Ui.Text("장바구니", true)); right.Controls.Add(orderAll);
        ...
        {
            var card = new ProductCard(p, data.AddToCart); productCards[p.Id] = card; products.Controls.Add(card);
        }
        products.ResumeLayout(false);
        search.TextChanged += (_, _) => FilterProducts(); category.SelectedIndexChanged += (_, _) => FilterProducts();
        supplier.SelectedIndexChanged += (_, _) => FilterProducts(); sort.SelectedIndexChanged += (_, _) => FilterProducts();
        data.CartChanged += SyncCart;
        FilterProducts(); SyncCart();
    }
    protected override void Dispose(bool disposing)
    {
        if (disposing) { data.CartChanged -= SyncCart; emptyCart.Dispose(); }
    ...
    private void FilterProducts()
    {
        ...
        count.Text = visible.Length == 0 ? "검색 결과가 없습니다." : $"상품 {visible.Length}개";
