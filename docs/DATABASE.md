...
필수 칸이 비고 `ProductUrl`이 있는 행은 MegaCoffee 공식 상품 URL만 조회한다. 검증된 상품명·가격·이미지·품절 상태를 사용하고, 카테고리는 분명한 명칭만 자동 분류하며 모호하면 `기타`로 둔다. `ProductId`가 있으면 해당 ID를 유지하고 수동 이미지는 보존한다. 모든 칸이 채워진 행은 웹 조회 없이 기존 가져오기 규칙을 따른다. DB와 파일 안의 신규 상품 중복은 조회 전에 건너뛰며, MegaCoffee는 판매처+`goodsNo`로 판별해 추적 파라미터 차이를 무시한다. 서로 다른 ProductId가 같은 상품을 수정하려 하면 오류로 중단한다. 조회 이미지는 확인 전 임시 저장 후 취소/오류 시 정리하며, 확인 후 백업과 단일 트랜잭션으로 반영한다. 지원하지 않는 URL·실제 오류가 있으면 DB를 바꾸지 않는다.
