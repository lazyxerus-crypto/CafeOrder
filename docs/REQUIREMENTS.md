...
현재는 [UI 7차 목업](UI.md#7차-목업-범위) 디자인에 상품/장바구니 SQLite 1단계, 상품 XLSX 입출력, MegaCoffee 로그인/세션 1단계를 연결한 상태다. 다른 판매처 로그인과 상품 조회·주문·결제 브라우저 자동화는 구현하지 않는다. 상품 우클릭 복사·Draft URL 기반 샘플 등록·로컬 UI/상품 상태 저장을 지원하고 내부 공급처 유형은 사용자 화면에 표시하지 않는다.
