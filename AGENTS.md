...
- 현재 UI 목업 단계의 허용 범위는 [UI 목업](docs/UI.md#7차-목업-범위)다. 수량/배송/완료는 샘플 메모리만 사용한다. 5차의 등록 상품·IsActive·카테고리·수동 이미지·UI 설정만 최소 로컬 파일로 저장한다. WebP 변환용 Magick.NET 이미지 라이브러리 하나를 허용한다. 쿠팡/네이버는 수동 완료 버튼 없이 주문 생성 확인이 확실할 때만 완료한다. 실제 DB 및 사이트 작업은 하지 않는다.
