...
- 7차 UI 디자인을 유지한다. 상품·장바구니는 로컬 SQLite에 저장하고 상품 XLSX는 검증·백업·단일 트랜잭션을 거친 입출력에만 사용한다. 창/열/글자 크기 설정은 기존 `ui-state.json`을 사용한다. MegaCoffee는 Playwright 로그인/세션, 사용자 URL 조회, XLSX의 URL 행 순차 조회만 지원한다. 다른 판매처 로그인·상품조회와 주문·배송은 여전히 목업 범위다. WebP 변환에는 Magick.NET을 사용한다. 자세한 전환 규칙은 [DATABASE](docs/DATABASE.md)를 따른다.
