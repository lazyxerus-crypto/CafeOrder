...
- 7차 UI 디자인을 유지한다. SQLite 1단계에서는 상품·장바구니만 로컬 DB에 저장하고, 창/열/글자 크기 설정은 기존 `ui-state.json`을 사용한다. 주문·배송·로그인·사이트 동작은 여전히 목업 범위다. WebP 변환에는 Magick.NET을 사용한다. 자세한 전환 규칙은 [DATABASE](docs/DATABASE.md)를 따른다.
