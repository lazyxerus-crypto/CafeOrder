# CafeOrder · WinForms UI 7차 + SQLite 1단계
...
검증기는 별도 임시 상태 경로에서 SQLite 4개 테이블/기존 JSON 1회 이전/재실행 복원/쓰기 실패 롤백/마이그레이션 백업과 기존 7차 UI 회귀 검사를 수행한다. 기존 클립보드는 복원한다. 결과/WinForms 렌더 캡처/성능 수치를 지정 폴더에 저장한다.
