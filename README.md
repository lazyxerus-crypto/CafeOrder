...
- 담기는 판매처 카드를 위로 이동시키고 잠깐 강조한다. +/-는 순서와 컨트롤을 유지한다. X는 즉시 삭제한다. 공통 토스트는 입력을 통과시키고 약 2.8초 후 사라진다.
...
- 네이버 연동 로그인은 실제 지원 대상으로 지정된 판매처가 없어 숨긴다. 판매처관리 상태색은 샘플이며 실제 로그인 결과가 아니다.
...
검증기는 전체 탭 최초 표시/재진입, 컨트롤 유지, 크기변경/최대화/복원, 스크롤/복사, 담기/삭제/품절 재확인, 주문 편집/잠금, 토스트 클릭/휠/포커스/만료를 검사한다. 로그 복사 검사 후 기존 클립보드를 복원한다. 결과와 화면 캡처·성능 수치를 지정 폴더에 저장한다.
